  ...
  for (let epoch = cursor; epoch < end; epoch++) {
   try {
    ...
    log(`epoch ${epoch}: ${leaves.length} records, root ${tree.root}, tx ${tx.hash}`);
      ...
      const fatal = err instanceof RecordError || /RecordError/.test(String(err));
      log(`epoch ${epoch} failed${fatal ? ' (UNCANONICALISABLE RECORD, sequence is stalled)' : ''}: ${err.message}`);
      return done;
   }
  }
