...
/**
 * Most periods a single run will try to catch up. A real backlog after an
 * outage is hours or days; anything past this means the contract and the
 * worker disagree about where epoch numbering starts.
 */
  ...
  for (const [col, field] of Object.entries(map)) {
    ...
    if (KINDS[kind].integers.includes(field)) {
      ...
      if (!Number.isSafeInteger(n)) {
        throw new Error(`column ${col} is not a safe integer: ${String(v)}`);
      }
      rec[field] = n;
    } else {
      rec[field] = String(v);
...
async function runOnce({ rpcUrl, privateKey, contract, fetch, now = Date.now, log = console.log }) {
  ...
  // The contract takes its origin from its own deployment block, and the
  // worker derives the period from the wall clock. If those two ever disagree
  // the loop below would try to walk decades of empty periods one transaction
  // at a time, so refuse rather than start.
  if (cursor < genesis) {
    throw new Error(`cursor ${cursor} is below genesisEpoch ${genesis}`);
  }
  if (currentEpoch - cursor > MAX_CATCHUP_EPOCHS) {
    throw new Error(
      `cursor ${cursor} is ${currentEpoch - cursor} periods behind the clock ` +
      `(limit ${MAX_CATCHUP_EPOCHS}). Refusing to run: this is a configuration ` +
      `error, not a backlog.`
    );
  }
