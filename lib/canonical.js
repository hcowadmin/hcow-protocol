 ...
 *
 *   seeded  A result that carries a seed commitment and the seed itself.
 *           Used by titles where the server picks the outcome. The record
 *           proves the revealed seed matches the commitment stored beside
 *           it, and that neither was changed after the anchor. It does not
 *           prove the commitment existed before play: both fields are
 *           written in the same row and anchored together after the round
 *           settled, so nothing on chain timestamps one before the other.
 *           Do not make that claim anywhere.
 *
...
const KINDS = {
  ...
  seeded: {
    ...
    fields: [
      ...
      'roundId',
      'playerRef',
      'serverSeedHash', // the seed commitment carried by the record
      'serverSeed',     // the seed itself, checked against the commitment
...
function canonicalize(rec, kind) {
  ...
  return spec.fields
    .map((f) => escape(spec.integers.includes(f) ? String(rec[f]) : rec[f]))
    .join('\t') + '\n';
}

/**
 * The decoder for the escaping `canonicalize` applies. Exported because the
 * injectivity argument for the whole leaf format rests on it existing.
 *
 * The leaf preimage joins nine fields with tabs and terminates with a newline,
 * and `abi.encodePacked` over adjacent variable-length operands is ambiguous
 * unless the boundary is recoverable. It is recoverable here because (A) no
 * escaped field can contain a raw tab or newline, so the delimiters are
 * exactly the field boundaries, and (B) the escaping is injective. B is only
 * credible with an explicit decoder, and this is it: `unescape(escape(f)) === f`
 * for every byte string f.
 *
 * The order matters and is the same order `canonicalize` escapes in. Escaping
 * the backslash last would send a literal backslash-t and a real tab to the
 * same two bytes, and this function could not tell them apart.
 *
 * @param {string} s an escaped field
 * @returns {string} the original field
 */
function unescape(s) {
  let out = '';
  for (let i = 0; i < s.length; i++) {
    ...
    else throw new RecordError(`bad escape sequence: \\${n}`);
  }
  return out;
}

/**
 * The escaping `canonicalize` applies to a single field.
 *
 * THE ORDER IS LOAD-BEARING. Three sequential replacements, so each one sees
 * the output of the previous: the backslash must be replaced first, or a
 * literal backslash-t in the input and a real tab both come out as the same
 * two bytes and `unescape` cannot tell them apart. The Solidity side is a
 * single pass over bytes with mutually exclusive branches and is therefore not
 * order dependent, which is why the differential test between the two matters
...
/**
 * Split a canonical form back into its nine fields. The inverse of the join,
 * and the executable half of the claim that the join is unambiguous.
 */
function decanonicalize(line) {
  if (!line.endsWith('\n')) throw new RecordError('canonical form must end with a newline');
  return line.slice(0, -1).split('\t').map(unescape);
}
 ...
 *
 * This proves the pair is internally consistent and, together with the
 * anchor, that neither field was changed afterwards. It does not prove the
 * commitment existed before the round was played, because both fields are
 * anchored together after it settled. Making the stronger claim in public
 * would be claiming something the chain does not show.
 */
...
module.exports = {
  KINDS, canonicalize, leafHash, seedCommitmentValid, recordTime, RecordError,
  // The decoder half. Exported so the injectivity claim can be tested rather
  // than asserted: see the lemma above HCOWLedger._join.
  escape, unescape, decanonicalize,
};
